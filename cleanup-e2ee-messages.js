...
const cleanupE2EEMessages = async () => {
	try {
		...
		console.log(
			`⚠️  Found ${vulnerableMessages.length} messages with BOTH plaintext and ciphertext`
		);
		...
		if (remainingVulnerable === 0) {
			...
			console.log(
				`⚠️  WARNING: ${remainingVulnerable} messages still have plaintext!`
			);
		...
		const e2eeMessages = await Message.countDocuments({
			encryptedText: { $exists: true, $ne: null, $ne: "" },
		});
		...
		console.log(
			`   E2EE messages: ${e2eeMessages} (${((e2eeMessages / totalMessages) * 100).toFixed(1)}%)`
		);
		console.log(
			`   Plaintext messages: ${plaintextMessages} (${((plaintextMessages / totalMessages) * 100).toFixed(1)}%)`
		);
