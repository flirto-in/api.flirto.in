...
export const uploadPrekeyBundle = asyncHandler(async (req, res) => {
	...
	console.log(
		`✅ Uploaded prekey bundle for user ${userId}: ${oneTimePrekeys.length} one-time keys`
	);
...
export const fetchPrekeyBundle = asyncHandler(async (req, res) => {
	...
	if (!bundle) {
		throw new ApiError(
			404,
			"User has not uploaded prekey bundle - E2EE not enabled"
		);
	}
	...
	if (bundle.oneTimePrekeys.length > 0) {
		...
		await bundle.save();
		console.log(
			`✅ Consumed one-time prekey ${oneTimePrekey.id} for session with ${peerUserId}`
		);
	} else {
		console.warn(
			`⚠️ No one-time prekeys available for ${peerUserId} - session will use 3DH instead of X3DH`
		);
	}
...
export const refreshOneTimePrekeys = asyncHandler(async (req, res) => {
	...
	if (!bundle) {
		throw new ApiError(
			404,
			"Prekey bundle not found - upload initial bundle first"
		);
	}
	...
	console.log(
		`✅ Refreshed ${oneTimePrekeys.length} one-time prekeys for user ${userId}`
	);
		...
		return res
			...
			.json(
				new ApiResponse(
					200,
					{ e2eeEnabled: false },
					"E2EE not enabled - no prekey bundle"
				)
			);
	}
	...
	return res.status(200).json(
		new ApiResponse(
			200,
			{
				...
				signedPrekeyAge: Math.floor(
					(Date.now() - bundle.signedPrekey.createdAt) / 86400000
				), // days
				oneTimePrekeyCount: bundle.oneTimePrekeys.length,
