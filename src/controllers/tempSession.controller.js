...
export const createTempSession = asyncHandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, { sessionId: session._id, code, alias }, 'Temp session created'));
});
...
export const joinTempSession = asyncHandler(async (req, res) => {
  ...
  const already = session.participants.some((p) => p.userId.toString() === userId.toString());
  if (!already) {
...
export const getTempSessionMessages = asyncHandler(async (req, res) => {
  ...
  if (!session || !session.active) throw new ApiError(404, 'Session not active');
  const messages = await Message.find({ tempSessionId: id }).sort({ createdAt: 1 });
  return res.status(200).json(new ApiResponse(200, { messages }, 'Temp messages loaded'));
});
...
export const endTempSession = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { sessionId: id }, 'Temp session ended and messages destroyed'));
});
