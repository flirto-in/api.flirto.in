...
export const searchUserByUid = asyncHandler(async (req, res) => {
	...
	const user = await User.findOne({ U_Id: uid }).select(
		"_id U_Id description online lastSeen"
	);
...
export const moveToPrimary = asyncHandler(async (req, res) => {
	...
	if (alreadyInPrimary) {
		return res
			.status(200)
			.json(new ApiResponse(200, {}, "User already in primary chat"));
	}
	...
	res
		.status(200)
		.json(new ApiResponse(200, {}, "Chat moved to primary successfully"));
});
...
export const moveToSecondary = asyncHandler(async (req, res) => {
	...
	if (alreadyInSecondary) {
		return res
			.status(200)
			.json(new ApiResponse(200, {}, "User already in secondary chat"));
	}
	...
	res
		.status(200)
		.json(new ApiResponse(200, {}, "Chat moved to secondary successfully"));
});
...
export const clearChat = asyncHandler(async (req, res) => {
	...
	res.status(200).json(new ApiResponse(200, {}, "Chat cleared successfully"));
});
...
export const muteChat = asyncHandler(async (req, res) => {
	...
	const isMuted = user.mutedChats.some((id) => id.toString() === userId);
...
export const uploadFileMessage = asyncHandler(async (req, res) => {
	...
	if (isTempRoom) {
		...
		const tempSession = await (
			await import("../models/TempSession.models.js")
		).default.findOne({ code, active: true });
		if (tempSession) {
	...
	return res
		.status(201)
		.json(new ApiResponse(201, { message: populated }, "File uploaded"));
});
