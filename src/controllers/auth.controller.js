...
const authUser = asyncHandler(async (req, res) => {
	...
	if (previousDeviceId && previousDeviceId !== newDeviceId) {
		...
		console.log(
			`🔄 User ${user.U_Id} logged in on new device. Previous device will be logged out.`
		);
	}
	...
	return res
		...
		.json(
			new ApiResponse(
				200,
				{ accessToken, user, previousDeviceId },
...
const tempSession = asyncHandler(async (req, res) => {
	...
	const user = await User.create({
		...
		U_Id,
		description: "Temporary guest session",
	});
	...
	return res
		.status(200)
		.json(
			new ApiResponse(200, { accessToken, user }, "Temporary session created")
		);
});
