    ...
    setInterval(async () => {
        try {
            ...
            for (const message of expiredMessages) {
                ...
                if (message.roomId) {
                    io.to(message.roomId).emit('message:self-destruct', { messageId: message._id });
                    ...
                    if (senderSocketId) {
                        io.to(senderSocketId).emit('message:self-destruct', { messageId: message._id });
                    }
                    if (receiverSocketId) {
                        io.to(receiverSocketId).emit('message:self-destruct', { messageId: message._id });
                    }
                }
            }
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('room:join', (data) => {
            ...
            if (roomId === 'public_temp' || roomId.startsWith('temp_room_')) {
                ...
                socket.emit('room:joined', { roomId, success: false, error: 'Invalid room' });
            }
        });
        ...
        socket.on('message:send', async (data) => {
            ...
            const { receiverId, roomId, text, encryptedText, iv, encryptedSessionKey, selfDestruct, tempSessionId, messageType, mediaUrl } = data;
                ...
                if (!roomId && receiverId) {
                    ...
                    console.log(`🔍 Checking chat initialization: sender=${socket.userId}, receiver=${receiverId}`);
                    ...
                    if (sender && receiver) {
                        ...
                        console.log(`📊 Sender chat status: primary=${isInSenderPrimaryChat}, secondary=${isInSenderSecondaryChat}`);
                        ...
                        console.log(`📊 Receiver chat status: primary=${isInReceiverPrimaryChat}, secondary=${isInReceiverSecondaryChat}`);
